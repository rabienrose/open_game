...
func _update_joystick(event_position: Vector2):
    ...
    if vector.length() > dead_size:
        ...
        if vector_mode == VectorMode.NORMALIZED:
            msg.joystick_output = vector.normalized()
            _center_control(_handle, msg.joystick_output * clamp_size + center)
        elif vector_mode == VectorMode.REAL:
            var clamped_vector := vector.clamped(clamp_size)
            msg.joystick_output = vector.normalized() * (clamped_vector.length() - dead_size) / (clamp_size - dead_size)
